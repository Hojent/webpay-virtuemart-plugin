...
public static $_this = FALSE;
	...
	public function getVmPluginCreateTableSQL () {
		...
		return $this->createTableSQL ('Payment Web Pay Table');
	}
	...
	function plgVmConfirmedOrder ($cart, $order) {
		...
		$html .= $this->getHtmlRow ('STANDART_ORDER_NUMBER', $order['details']['BT']->order_number, "vmorder-done-nr");
		...
		//$wsb_notify_url = JROUTE::_ (JURI::root () . 'index.php?option=com_virtuemart&view=pluginresponse&task=pluginnotification&tmpl=component&total='.$order['details']['BT']->order_total);
        ...
        $wsb_currency_id='BYR';
		$SecretKey=$wsbPsw;
        $sigma = $wsb_seed.$wsbID.$wsb_order_num.$wsb_test.$wsb_currency_id.$wsb_total.$SecretKey;
		$wsb_signature = sha1($wsb_seed.$wsbID.$wsb_order_num.$wsb_test.$wsb_currency_id.$wsb_total.$SecretKey);
		//$wsb_signature="40aa8af3ce7a1f7c3635ac2227475094a59d574c";


		/* Форма передачи данных в банк */
		$html.= '<form id="checkout" name="checkout" method="post"' ;
		$html.= 'action="'.$p['wsb_payurl'].'"';
		$html.='>';
		...
		$html.= '<input type="hidden" name="wsb_storeid" value="'.$wsbID.'"/>';
		$html.= '<input type="hidden" name="wsb_store" value="'.$wsbName.'"/>';
		$html.= '<input type="hidden" name="wsb_order_num" value="'.$OrderID.'"/>';
		$html.= '<input type="hidden" name="wsb_test" value="0"/>';
		$html.= '<input type="hidden" name="wsb_currency_id" value="'.$wsb_currency_id.'"/>';
		$html.= '<input type="hidden" name="wsb_seed" value="'.$wsb_seed.'"/>';
		$html.= '<input type="hidden" name="wsb_return_url" value="'.$wsb_return_url.'"/>';

		$html.= '<input type="hidden" name="wsb_cancel_return_url" value="'.$wsb_cancel_return_url.'"/>';
		$html.= '<input type="hidden" name="wsb_notify_url" value="'.$wsb_notify_url.'"/>';
		$html.= '<input type="hidden" name="wsb_email" value=""/>';
		$html.= '<input type="hidden" name="wsb_phone" value=""/>';


		$i = 0;
		foreach ($cart->products as $key => $product)
		{
		$html.= '<input type="hidden" name="wsb_invoice_item_name['.$i.']" value="'.$product->product_name.'"/>';
		$html.= '<input type="hidden" name="wsb_invoice_item_quantity['.$i.']" value="'.$product->quantity.'"/>';
		$vrem_summ=$cart->pricesUnformatted[$key]['salesPrice'];
		$vrem_summ=$vrem_summ*$currency_exchange_rate; //  это мы принудительно пересчитали в BYR
		$html.= '<input type="hidden" name="wsb_invoice_item_price['.$i.']" value="'.$vrem_summ.'"/>';
        $i++;
		}


		$html.= '<input type="hidden" name="wsb_total" value="'.$wsb_total.'"/>';
		$html.= '<input type="hidden" name="wsb_signature"	value="'.$wsb_signature.'"/>';
		$html.= '<input type="hidden" name="wsb_tax" value="'.$wsb_tax.'"/>';
		$html.= '<input type="hidden" name="wsb_shipping_name" value="'.$wsb_shipping_name.'"/>';
		$html.= '<input type="hidden" name="wsb_shipping_price" value="'.$wsb_shipping_price.'"/>';
		$html.= '<input type="hidden" name="wsb_discount_name" value="'.$wsb_discount_name.'"/>';
		$html.= '<input type="hidden" name="wsb_discount_price" value="'.$wsb_discount_price.'"/>';
	...
	function plgVmOnPaymentResponseReceived (&$html) {
		...
		$payment_name = ' Система Web Pay BY ';
